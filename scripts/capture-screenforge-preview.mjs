...
const OUTPUT_DIR =
	process.env.SCREENFORGE_PREVIEW_OUTPUT_DIR ??
	path.join(os.tmpdir(), `screenforge-real-preview-${Date.now()}`);
const FRAME_COUNT = Number(process.env.SCREENFORGE_PREVIEW_FRAME_COUNT ?? 90);
const FPS = Number(process.env.SCREENFORGE_PREVIEW_FPS ?? 30);
...
function findLatestCursorRecordingData() {
	...
	const candidates = fs
		...
		.filter((entry) => entry.isDirectory() && entry.name.startsWith("screenforge-cursor-native-"))
...
function runNpmBuildViteIfRequested() {
	if (process.env.SCREENFORGE_PREVIEW_SKIP_BUILD === "true") {
		ensureBuildExists();
...
const fixtureVideoPath = path.join(OUTPUT_DIR, "screenforge-preview-fixture.webm");
const outputVideoPath = path.join(OUTPUT_DIR, "screenforge-preview.webm");
...
try {
	...
	await hudWindow.evaluate(async () => {
		...
		throw new Error("Timed out waiting for ScreenForge IPC handlers.");
	});
