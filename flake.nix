{
  description = "ScreenForge — desktop screen recorder with built-in editor";
  ...
  outputs =
    ...
    in
    {
      ...
      packages = forAllSystems (pkgs: {
        screenforge = pkgs.callPackage ./nix/package.nix { };
      ...
      devShells = forAllSystems (
        ...
        in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              echo "ScreenForge dev shell — node $(node --version), electron v$(electron --version 2>/dev/null | tr -d 'v')"
            '';
          };
        }
      );
      ...
      overlays.default = final: _prev: {
        screenforge = self.packages.${final.stdenv.hostPlatform.system}.screenforge;
      };
