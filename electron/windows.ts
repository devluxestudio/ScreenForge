...
export function createHudOverlayWindow(): BrowserWindow {
	...
	// Exclude the recording pill from the captured video on both platforms:
	//   Windows → SetWindowDisplayAffinity(WDA_EXCLUDEFROMCAPTURE) via WGC / DXGI
	//   macOS   → NSWindow content-protection layer (excluded from screenshots)
	// The operator still sees the pill on screen; it is simply invisible to the
	// screen-capture API so it never appears in the recorded video.
...
export function createCountdownOverlayWindow(): BrowserWindow {
	...
	// Also exclude the countdown overlay from screen capture (same logic as HUD).
	if (process.platform === "darwin") {
		win.setContentProtection(true);
	}
