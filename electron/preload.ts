...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	// Called before browser-fallback getDisplayMedia to tell the main process
	// which cursor mode to use so it can suppress the native cursor if needed.
	setPendingCursorCaptureMode: (mode: import("../src/lib/recordingSession").CursorCaptureMode) => {
		return ipcRenderer.invoke("set-pending-cursor-capture-mode", mode);
	},
	...
	writeExportToPath: (videoData: ArrayBuffer, filePath: string, thumbnailPath?: string) => {
		return ipcRenderer.invoke("write-export-to-path", videoData, filePath, thumbnailPath);
	},
	openVideoFilePicker: () => {
	...
	startKeystrokeCapture: (recordingStartMs: number) => {
		return ipcRenderer.invoke("start-keystroke-capture", recordingStartMs);
	},
