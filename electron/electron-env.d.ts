...
interface Window {
	electronAPI: {
		...
		/** Tells the main process which cursor mode the renderer will use for the upcoming
		 *  browser-fallback getDisplayMedia call, so setDisplayMediaRequestHandler can
		 *  suppress the native cursor when using editable-overlay mode. */
		setPendingCursorCaptureMode: (
			mode: import("../src/lib/recordingSession").CursorCaptureMode,
		) => Promise<void>;
