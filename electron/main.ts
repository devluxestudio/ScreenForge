...
appReady?.then(async () => {
	...
	session.defaultSession.setDisplayMediaRequestHandler(
		(request, callback) => {
			...
			// When the renderer uses editable-overlay cursor mode, suppress the native
			// cursor from being baked into the browser-recorded video. This prevents
			// the "double cursor" effect (system cursor + custom overlay cursor).
			const suppressCursor = getPendingCursorCaptureMode() === "editable-overlay";
			callback({
				video: source,
				...(request.audioRequested && process.platform === "win32" ? { audio: "loopback" } : {}),
				// enableLocalEcho false = don't render the mouse cursor into the capture stream.
				...(suppressCursor ? { enableLocalEcho: false } : {}),
