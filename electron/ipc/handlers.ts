...
import { existsSync, constants as fsConstants, mkdirSync } from "node:fs";
...
const PROJECT_FILE_EXTENSION = "screenforge";
...
function resolveRecordingOutputPath(fileName: string): string {
	...
	return path.join(projectDir, parsedPath.base);
}
...
// Tracks the cursor mode the renderer is about to use for browser-fallback getDisplayMedia.
// Set by the renderer just before calling getDisplayMedia so that the
// setDisplayMediaRequestHandler in main.ts can suppress the native cursor.
let pendingCursorCaptureMode: CursorCaptureMode = "editable-overlay";
...
// Returns the cursor mode that the renderer intends to use for the upcoming browser-fallback
// getDisplayMedia call. Used by setDisplayMediaRequestHandler to suppress the native cursor.
export function getPendingCursorCaptureMode(): CursorCaptureMode {
	return pendingCursorCaptureMode;
}
...
async function readKeystrokeTelemetryFile(
	targetVideoPath: string,
): Promise<import("../../src/native/contracts").KeystrokeRecordingData> {
	const keystrokesPath = `${targetVideoPath}.keystrokes.json`;
	try {
		const content = await fs.readFile(keystrokesPath, "utf-8");
		const parsed = JSON.parse(content);
		const rawEvents = Array.isArray(parsed?.events) ? parsed.events : [];
		const events = rawEvents
			.filter(
				(e: unknown): e is { timeMs: number; keys: string[] } =>
					typeof (e as Record<string, unknown>)?.timeMs === "number" &&
					Array.isArray((e as Record<string, unknown>)?.keys),
			)
...
function getNativeWindowsCaptureHelperCandidates() {
	const envPath = process.env.SCREENFORGE_WGC_CAPTURE_EXE?.trim();
...
function getNativeMacCaptureHelperCandidates() {
	const envPath = process.env.SCREENFORGE_SCK_CAPTURE_EXE?.trim();
	const archTag = process.arch === "arm64" ? "darwin-arm64" : "darwin-x64";
	const helperName = "screenforge-screencapturekit-helper";
	return [
		envPath,
...
export function registerIpcHandlers(
	...
	async function requestScreenAccess() {
		...
		try {
			...
			// TCC prompt without opening ScreenForge's source selector above it.
	...
	// Called by the renderer just before browser-fallback getDisplayMedia so that
	// setDisplayMediaRequestHandler in main.ts can suppress the native cursor when
	// using editable-overlay mode.
	ipcMain.handle("set-pending-cursor-capture-mode", (_, mode: CursorCaptureMode) => {
		pendingCursorCaptureMode = normalizeCursorCaptureMode(mode) ?? "editable-overlay";
	});
		...
		if (process.platform === "darwin" && !access.granted) {
			...
			const detail =
				access.status === "missing-helper"
					...
					: "Allow ScreenForge under System Settings → Privacy & Security → Accessibility, then press record again to start the countdown.";
		...
		if (!access.granted) {
			if (process.platform === "darwin" && access.status !== "not-determined") {
				...
				const messageOptions = {
					...
					detail:
						"Allow ScreenForge in macOS System Settings, then come back and choose a screen or window.",
				} satisfies Electron.MessageBoxOptions;
				const result =
					mainWin && !mainWin.isDestroyed()
						? await dialog.showMessageBox(mainWin, messageOptions)
	...
	ipcMain.handle("start-keystroke-capture", (_, recordingStartMs: number) => {
		if (!keystrokeRecorder) {
			keystrokeRecorder = new KeystrokeRecorder();
		}
		keystrokeRecorder.start(recordingStartMs);
	});

	ipcMain.handle("stop-keystroke-capture", async (_, videoPath: string) => {
		if (!keystrokeRecorder) return;

		const data = keystrokeRecorder.stop();
		if (data.events.length > 0) {
			try {
				const keystrokesPath = `${videoPath}.keystrokes.json`;
				await fs.writeFile(keystrokesPath, JSON.stringify(data, null, 2), "utf-8");
			} catch (error) {
				console.error("Failed to save keystrokes telemetry:", error);
			}
		}
	});
	...
	ipcMain.handle(
		...
		async (_, request: NativeWindowsRecordingRequest) => {
			try {
				...
				const projectDir = path.join(RECORDINGS_DIR, `${RECORDING_FILE_PREFIX}${recordingId}`);
				if (!existsSync(projectDir)) {
					mkdirSync(projectDir, { recursive: true });
				}

				const outputPath = path.join(projectDir, `${RECORDING_FILE_PREFIX}${recordingId}.mp4`);
	...
	ipcMain.handle("get-recorded-video-path", async () => {
		try {
			...
			for (const entry of entries) {
				if (entry.isDirectory() && entry.name.startsWith("recording-")) {
					const dirPath = path.join(RECORDINGS_DIR, entry.name);
					const files = await fs.readdir(dirPath);
					const videoFile = files.find((f) => f.endsWith(".webm") && !f.endsWith("-webcam.webm"));
					if (videoFile) {
						videoFiles.push(path.join(entry.name, videoFile));
					}
				} else if (
					entry.isFile() &&
					entry.name.endsWith(".webm") &&
					!entry.name.endsWith("-webcam.webm")
				) {
			...
			// Lexicographical sort works because of the timestamp prefix
	...
	async function loadProjectFileFromPath(filePath: string): Promise<ProjectFileResult> {
		try {
			...
			if (path.extname(filePath).toLowerCase() !== `.${PROJECT_FILE_EXTENSION}`) {
				return { success: false, message: "Not a ScreenForge project file" };
			}
	...
	ipcMain.handle(
		...
		async (
			...
			const { filePath, canceled } = await dialog.showSaveDialog({
				...
				defaultPath: `screenforge-diagnostic-${Date.now()}.json`,
	...
	registerNativeBridgeHandlers({
		...
		loadKeystrokeTelemetry: readKeystrokeTelemetryFile,
	});
}
