...
export interface NativeBridgeContext {
	...
	loadKeystrokeTelemetry: (videoPath: string) => Promise<KeystrokeRecordingData>;
}
	...
	ipcMain.handle(NATIVE_BRIDGE_CHANNEL, async (_, request: unknown) => {
		...
		try {
			switch (request.domain) {
				...
				case "keystroke": {
					const keystrokeRequest = request as {
						domain: "keystroke";
						action: string;
						payload?: { videoPath?: string };
						requestId?: string;
					};
					const action = keystrokeRequest.action;
					const videoPath = keystrokeRequest.payload?.videoPath;
					switch (action) {
						case "getTelemetry": {
							if (!videoPath) {
								return createSuccessResponse(requestId, { version: 1, events: [] });
							}
							const resolvedPath = context.resolveVideoPath(videoPath);
							if (!resolvedPath) {
								return createSuccessResponse(requestId, { version: 1, events: [] });
							}
							return createSuccessResponse(
								requestId,
								await context.loadKeystrokeTelemetry(resolvedPath),
							);
						}
						default:
							return createErrorResponse(
								requestId,
								"UNSUPPORTED_ACTION",
								`Unsupported keystroke action: ${action}`,
							);
					}
				}
