...
type UseScreenRecorderReturn = {
	...
	setKeystrokeCaptureEnabled: (enabled: boolean) => void;
};
...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const [keystrokeCaptureEnabled, setKeystrokeCaptureEnabled] = useState(true);
		...
		(
			...
			void (async () => {
				...
				try {
					...
					const finalPath = result.session?.screenVideoPath || result.path;
					if (finalPath) {
						await window.electronAPI.stopKeystrokeCapture(finalPath);
					}
	...
	const finalizeNativeWindowsRecording = useCallback(
		async (discard = false) => {
			...
			try {
				...
				const finalPath = storedSession?.screenVideoPath || nativeScreenPath;
				if (finalPath) {
					await window.electronAPI.stopKeystrokeCapture(finalPath);
				}
	...
	const startNativeWindowsRecordingIfAvailable = async (
		...
		try {
			...
			setPaused(false);
			setElapsedSeconds(0);
			if (keystrokeCaptureEnabled && window.electronAPI?.startKeystrokeCapture) {
				window.electronAPI.startKeystrokeCapture(segmentStartedAt.current);
			}
			return true;
		} catch (error) {
	...
	const startRecording = async (countdownRunToken?: number) => {
		try {
			...
			if (platform === "win32") {
				// Notify the main process of the cursor mode so setDisplayMediaRequestHandler
				// can suppress the native cursor when using editable-overlay mode,
				// preventing the "double cursor" (baked system cursor + custom overlay).
				await window.electronAPI.setPendingCursorCaptureMode(cursorCaptureMode);
