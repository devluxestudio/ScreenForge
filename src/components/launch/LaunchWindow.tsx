		...
		>
			...
			>
				...
				>
					...
					<button
						data-testid="launch-keystroke-button"
						className={`${hudIconBtnClasses} ${keystrokeCaptureEnabled ? "drop-shadow-[0_0_4px_rgba(74,222,128,0.4)]" : ""}`}
						onClick={() => !recording && setKeystrokeCaptureEnabled(!keystrokeCaptureEnabled)}
						disabled={recording}
						title={keystrokeCaptureEnabled ? t("keystroke.disable") : t("keystroke.enable")}
					>
						{getIcon("keyboard", keystrokeCaptureEnabled ? "text-green-400" : "text-white/40")}
					</button>
				</div>
