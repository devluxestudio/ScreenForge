	...
	const fetchSources = useCallback(async () => {
		...
		try {
			const rawSources = await window.electronAPI.getSources({
				types: ["screen", "window"],
				thumbnailSize: isWindows ? { width: 0, height: 0 } : { width: 320, height: 180 },
				fetchWindowIcons: !isWindows,
			});
			setSources(
		...
		return (
			...
			>
				<div className="text-center">
					<div className="animate-spin duration-500 rounded-[50%] h-6 w-6 border-2 border-b-transparent border-[#000AF2] mx-auto mb-2" />
			...
			>
				<div className="max-w-[320px] px-6 text-center">
					...
					<Button
						...
						className="mt-4 h-8 rounded-lg bg-[#000AF2] px-5 text-[11px] font-semibold text-white transition-transform duration-150 hover:bg-[#000AF2]/85 active:scale-95"
					>
						{tc("actions.reload")}
					</Button>
				</div>
		...
		<div className={`min-h-screen flex flex-col ${styles.glassContainer}`}>
			...
			<div className="flex justify-center gap-2 border-t border-white/[0.06] p-3">
				...
				<Button
					...
					className="h-8 rounded-lg bg-[#000AF2] px-5 text-[11px] font-semibold text-white transition-transform duration-150 hover:bg-[#000AF2]/85 active:scale-95 disabled:bg-zinc-700 disabled:opacity-30"
				>
					{tc("actions.share")}
				</Button>
			</div>
