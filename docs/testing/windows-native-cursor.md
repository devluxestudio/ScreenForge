...
This script does not launch ScreenForge. It:
...
```text
C:\Users\<user>\AppData\Local\Temp\screenforge-cursor-native-...
```
...
$env:CURSOR_TEST_OUTPUT_DIR = "C:\temp\screenforge-cursor-test"
npm run test:cursor-native:win
```

## ScreenForge preview capture

```powershell
npm run capture:screenforge-preview
```

This script launches the real Electron app, injects a fixture video plus cursor sidecar data, opens the editor, captures frames from the actual ScreenForge preview UI, and encodes them into a WebM.
...
```powershell
$env:SCREENFORGE_PREVIEW_SKIP_BUILD = "true"
$env:SCREENFORGE_PREVIEW_FRAME_COUNT = "120"
$env:SCREENFORGE_PREVIEW_FPS = "30"
$env:SCREENFORGE_PREVIEW_OUTPUT_DIR = "C:\temp\screenforge-preview"
npm run capture:screenforge-preview
```

Useful files:

- `screenforge-preview.webm`: video of the real ScreenForge editor preview
...
- whether the real ScreenForge preview renders the same cursor behavior as the diagnostic pipeline
...
Build ScreenForge's helper locally:
...
For local diagnostics with another compatible helper, point ScreenForge at that executable:

```powershell
$env:SCREENFORGE_WGC_CAPTURE_EXE = "C:\path\to\wgc-capture.exe"
npm run build-vite
npm run dev
```
